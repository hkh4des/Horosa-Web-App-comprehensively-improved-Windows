...
这个目录承载 Horosa v2.1.3 Beta Windows 桌面应用工程。

- 当前发布版本：`2.1.3 Beta`（安装器版本号为 `2.1.3`）
- 发布定位：Mac 2.1.0 beta 产品面同步后的 Windows Web / Desktop 自包含大版本
- 用户入口：`release/Horosa-Setup-2.1.3.exe`
...
v2.1.3 Beta 对齐 Mac 2.1.0 beta 的最新产品面，但 Windows 发布包必须保持自包含。旧的 `Horosa-Web-App-comprehensively-improved-MacOS-main` 同步来源文件夹不应作为安装版、Web 启动或打包依赖。
...
公开下载入口是 `Horosa-Setup-2.1.3.exe`。`latest.yml`、`.blockmap` 与 `SHA256SUMS.txt` 用于更新和校验流程。
