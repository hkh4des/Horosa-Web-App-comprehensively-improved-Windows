...
## 1. 各盘符号几何上偏(根因级)
- 根因:`AstroChartCircle`/`AstroHelper` 的所有图表文字用 `dominant-baseline:"middle"`。对天文符号字体,`middle` 到 x-height 中线而非几何中心,导致符号整体上偏(实测 ~7.9px @24px)。
- 改动:把这两个文件里全部 `dominant-baseline","middle"` → `"central"`(central 按字体几何中心)。`AstroChartCircle` 是占星/合盘共享渲染器、`AstroHelper` 是共享绘制助手,故覆盖占星(清简+经典)/合盘/辅盘/主限法盘等所有西洋盘。
