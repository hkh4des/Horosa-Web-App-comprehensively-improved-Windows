# Horosa Windows 2.4.0 Beta 项目结构说明

当前目录结构对应 Horosa v2.4.0 Beta Windows Web / Desktop 统一交付形态。这个版本已经把 Mac 2.1.0 beta 的 UI、前端、Python 后端、Java 后端、资源、AI 导出、传统命法/卜法后端与桌面交付修复同步到 Windows 本地工程；旧的 `Horosa-Web-App-comprehensively-improved-MacOS-main` 同步来源文件夹不再是运行或构建依赖。

## 普通用户入口

- `START_HERE.bat`：仓库根目录的一键 Web 启动入口，适合解压式本地运行和维护自检。
- `desktop_installer_bundle/release/Horosa-Setup-2.4.0.exe`：正式 Windows 安装包入口，适合 GitHub Release 发布。
- `README.md` / `README_ZH.md` / `README_EN.md`：v2.4.0 Beta 用户说明、截图、功能介绍和下载入口。
- `docs/给完全不会的人看的启动说明.txt`：完全不会命令行的用户说明。

## 根目录主要内容

- `README.md`：双语入口页，展示新版 icon、截图、v2.4.0 Beta 功能介绍和下载入口。
...
- `docs/releases/2.4.0.md`：当前 Beta 版本发布说明（西占六技法全链路 AI + 容许度 orbs 存档 + 新增 /predict/dist · /predict/agepoint · /astroextra/greatconj 路由，重建 jar）。（Issue #10 服务不稳定修复：AI 流式 SSE 线程安全 SseChannel + SSE 标志跨请求污染 / async 重派发验签）。
- `docs/releases/2.3.0.md`：上一版 Beta 发布说明（占星地图 ACG 升级 + 卜卦盘/择日盘 + 河洛理数补全 + AI 系统代理 Issue #9 + 六壬发三传顺序/占星右栏/风水UI/快捷dock 多项修复），保留作历史记录。
- `docs/releases/2.2.1.md`：上一版 Beta 发布说明（全局日界点·晚子时第二开关 + AI 供应商连通性 + Windows 应用内自动更新启用 + 本地 AI 流式 Issue #8 修复），保留作历史记录。
- `docs/releases/2.2.0.md`：上一版 Beta 发布说明（数算两新技法 邵子参评数/河洛理数 + 调波盘绘制 + 风水 React 重写 + AI 模型选择修复；含 Windows issue #7 内置 tar 解析修复），保留作历史记录。
- `docs/releases/2.1.8.md`：上一版 Beta 发布说明（UI 符号居中 + 太乙四柱时间 + 主限推运年数 + Ollama 流式[issue #6] + 八字月柱交节边界 + Tier-1 减重 ~29%），保留作历史记录。
- `docs/releases/2.1.7.md`：更早的 Beta 发布说明（奇门/三式 真太阳时定盘修复），保留作历史记录。
- `docs/releases/2.1.6.md`：更早的 Beta 发布说明（奇门历法修复：月柱交节边界 + 置闰超神接气定局；印度盘地图选点修复；Windows 侧 issue #2 内置运行时环境隔离），保留作历史记录。
- `docs/releases/2.1.5.md`：更早的 Beta 发布说明（AI 分析页全面修复：供应商切换/鉴权 + 发送安全 + 静默失败透出），保留作历史记录。
- `docs/releases/2.1.4.md`：更早的 Beta 发布说明（AI 供应商兼容 + 错误透传 + 凭据脱敏），保留作历史记录。
- `docs/releases/2.1.3.md`：更早的 Beta 发布说明（八字时间显示修复），保留作历史记录。
- `docs/releases/2.1.2.md`：更早的 Beta 发布说明（AI 分析同步），保留作历史记录。
- `docs/releases/2.1.1.md`：更早的 Beta 发布说明，保留作历史记录。
- `docs/releases/2.0.1.md`：更早的 Beta 发布说明，保留作历史记录。
...
- `local/workspace/Horosa-Web-55c75c5b088252fbd718afeffa6d5bcb59254a0c/astrostudyui/`：v2.4.0 Beta 前端工程与构建产物。
...
- `package.json` / `package-lock.json`：桌面打包工程版本与依赖，当前发布版本为 `2.4.0`。
...
- `release/`：本地构建输出目录。v2.4.0 Beta 标准发布资产为：
...
- GitHub Release 正式用户入口是 `Horosa-Setup-2.4.0.exe`。
...
- 改 logo、加 Python 依赖、动启动/编排逻辑时，参照 `docs/CLEAN_MACHINE_NATIVE_RUNTIME_FIX.md` 的“以后怎么做 / 注意点”；构建期 `stage-runtime.cjs` 会自动跑 `check_runtime_native_deps.py`，缺失非系统 DLL 会直接让构建失败。
