# Horosa Windows 2.2.0 Beta 项目结构说明

当前目录结构对应 Horosa v2.2.0 Beta Windows Web / Desktop 统一交付形态。这个版本已经把 Mac 2.1.0 beta 的 UI、前端、Python 后端、Java 后端、资源、AI 导出、传统命法/卜法后端与桌面交付修复同步到 Windows 本地工程；旧的 `Horosa-Web-App-comprehensively-improved-MacOS-main` 同步来源文件夹不再是运行或构建依赖。

## 普通用户入口

- `START_HERE.bat`：仓库根目录的一键 Web 启动入口，适合解压式本地运行和维护自检。
- `desktop_installer_bundle/release/Horosa-Setup-2.2.0.exe`：正式 Windows 安装包入口，适合 GitHub Release 发布。
- `README.md` / `README_ZH.md` / `README_EN.md`：v2.2.0 Beta 用户说明、截图、功能介绍和下载入口。
- `docs/给完全不会的人看的启动说明.txt`：完全不会命令行的用户说明。

## 根目录主要内容

- `README.md`：双语入口页，展示新版 icon、截图、v2.2.0 Beta 功能介绍和下载入口。
...
- `docs/releases/2.2.0.md`：当前 Beta 版本发布说明与发布资产清单（数算两新技法 邵子参评数/河洛理数 + 调波盘绘制 + 风水 React 重写 + AI 模型选择修复；含 Windows issue #7 内置 tar 解析修复）。
...
- `local/workspace/Horosa-Web-55c75c5b088252fbd718afeffa6d5bcb59254a0c/astrostudyui/`：v2.2.0 Beta 前端工程与构建产物。
...
- `release/`：本地构建输出目录。v2.2.0 Beta 标准发布资产为：
...
- Mac 同步来源文件夹只作为迁移参考，v2.2.0 Beta 发布工程自身不应依赖它。
