...
- `components/auxchart/AuxChartMain.js`：给 `<AstroHarmonicLab>` 补传 `planetDisplay/chartDisplay/lotsDisplay/showAstroMeaning`（此前只传了 `value/height`），使盘按左栏星显选择绘制（ germany/hellen 兄弟）。
